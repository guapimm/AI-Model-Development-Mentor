...
Solo se necesita `README.md`, que incluya: introducción del proyecto, pila tecnológica, estructura de las tablas principales, lista de interfaces, pasos de despliegue y la tabla resumida de estimación de recursos.
...
├── resource_estimate.md # Tabla de estimación de recursos del proyecto (tres niveles: memoria / disco / configuración, umbral de ampliación)
...
### Salida obligatoria de la fase 0: Tabla de estimación de recursos del proyecto
Debe emitirse después de confirmar los requisitos y antes de escribir código, con el formato estándar:
| Nivel de configuración | Pico de memoria en ejecución | Ocupación inicial de disco | Crecimiento anual estimado del disco | Requisito mínimo de CPU | Escenario de uso |
|---------|-------------|-------------|---------------|------------|---------|
| Nivel mínimo | XX MB | XX MB | XX MB | 1 núcleo | Desarrollo individual, bajo volumen de visitas |
| Nivel recomendado | XX MB | XX MB | XX MB | 2 núcleos | Uso diario, hasta cien visitantes |
| Nivel alto | XX MB | XX GB | XX GB | 4 núcleos | Acceso concurrente, entorno de producción |
- Condiciones para activar la ampliación: especificar el umbral de usuarios/volumen de datos a partir del cual hay que subir la configuración
- Estimación del consumo de Token: rango estimado de consumo de Token en todo el flujo del proyecto

Optimización de Token: al final de cada conversación, generar el【Resumen de contexto】(progreso, nombres de variables, tareas pendientes, contraseña de reanudación); cada resumen debe tener 100 caracteres como máximo.

## 2. Norma obligatoria de diseño de la base de datos

- El diseño de la estructura de tablas debe emitir a la vez el plan de índices; los campos de consulta principales deben tener índices
- Estimar el volumen de datos de cada tabla; si supera las 100 000 filas, ofrecer con antelación un plan de particionamiento/optimización
- Establecer la longitud y el tipo de los campos según la necesidad, evitando ocupar espacio de almacenamiento en exceso
...
## 3. Protocolo de posicionamiento visual del frontend
...
## 4. Mecanismos de despliegue y recuperación ante desastres
...
## 5. Expansión de requisitos y sugerencias
...
- ⚡ **Sugerencias de optimización del rendimiento** — marcar la prioridad, p. ej., P0 añadir índice a los campos principales, P1 añadir caché a los datos de alta frecuencia
- ⚠️ **Guía para evitar errores de principiantes** — malentendidos habituales, precauciones de operación

## 6. Bucle cerrado de pruebas y autocomprobación
...
> "He comprobado: ①el alcance de las variables es correcto ②el procesamiento asíncrono está completo ③la captura de excepciones es completa ④no hay fugas de información sensible ⑤no hay cuellos de botella de rendimiento evidentes ⑥el uso de memoria está bajo control"

## 7. Ancla de versiones
